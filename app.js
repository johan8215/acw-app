  ...
  try {
    ...
    if (!data.ok) {
      document.getElementById("schedule").innerHTML =
        `<p style="color:#b30000;">No schedule found.</p>`;
      return;
    }
    ...
    console.error(e);
    document.getElementById("schedule").innerHTML =
      `<p style="color:#b30000;">Connection error.</p>`;
  }
}
...
/* ---------- Employee Modal + WhatsApp ---------- */
let currentEmp=null;
function openEmployee(emp){
  currentEmp=emp;
  const m=document.getElementById("employeeModal"); m.style.display="flex";
...
async function sendEmpMessage(){
  ...
  const digits=(currentEmp.phone||"").replace(/\D/g,"");
